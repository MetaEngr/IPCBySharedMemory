...
class SHM_IPC_EXPORT ShmIPCClient
{
public:
    explicit ShmIPCClient(IShmChannelNotify* notify);
    ~ShmIPCClient();
...
private:
    ShmIPCClient(const ShmIPCClient&) = delete;
    ShmIPCClient& operator=(const ShmIPCClient&) = delete;
...
private:
    ...
    // 线程
    ShmIPCWriter* shmClientMsgWriter_ = nullptr;
    ShmIPCReader* shmServerMsgReader_ = nullptr;
